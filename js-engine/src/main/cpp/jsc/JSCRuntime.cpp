...
#ifdef USE_QYRNLOG
    QYRNLogCxx::log_oc_msg(std::string("JSCRuntime::evaluateJavaScript begin JSEvaluateScript"));
#endif
  JSValueRef res =
      JSEvaluateScript(ctx_, sourceRef, nullptr, sourceURLRef, 0, &exc);
#ifdef USE_QYRNLOG
    QYRNLogCxx::log_oc_msg(std::string("JSCRuntime::evaluateJavaScript end JSEvaluateScript"));
#endif
